...
public class ExprPortalTravelFind extends SimpleExpression<Location> {
    ...
    protected Location[] get(Event event) {
		...
		if (event instanceof PlayerPortalEvent) {
			c = ((PlayerPortalEvent) event).getPortalTravelAgent().findPortal(to);
			return new Location[] { c };
		} else if (event instanceof EntityPortalEvent) {
			c = ((EntityPortalEvent) event).getPortalTravelAgent().findPortal(to);
			return new Location[] { c };
		}
        return new Location[] { c };
    }
